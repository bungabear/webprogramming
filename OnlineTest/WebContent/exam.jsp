...
<script>
	...
	var x = setInterval(function() {
		...
		document.getElementById("timer").innerHTML ="남은 시험 시간 : "+ seconds + "초 ";
		if (distance < 1000){
			clearInterval(x);
...
<body>
	<center>
		<form name="exam_form" action="exam_ok.jsp" method="post">
			<h2><p id="timer"/></h2><br>
			<br> 학번:<input type="text" name="studentNum" required>
		...
		return str;
				...
				<tr>
					<td rowspan=2><h3>
							문제<%=number%><br><input type="hidden" name="question<%=number%>" value="<%=qNum%>"></h3></td>
					<td colspan=4><%=question%> <%if (type1 == 'I'){%><br><img src="image/<%=qImage%>"><%} %></td>
				</tr>
				<tr>
					<td><input type="radio" name="answer<%=number%>" value="<%=ex[0]%>"><%=check(ex[0], type2)%></td>
					<td><input type="radio" name="answer<%=number%>" value="<%=ex[1]%>"><%=check(ex[1], type2)%></td>
					<td><input type="radio" name="answer<%=number%>" value="<%=ex[2]%>"><%=check(ex[2], type2)%></td>
					<td><input type="radio" name="answer<%=number%>" value="<%=ex[3]%>"><%=check(ex[3], type2)%></td>
				</tr>
				<%
					number++;
						}
